...
export function osmAuth(o) {
  ...
  oauth.authenticated = function() {
    ...
    return !!token('basic_auth_token');// TODO
  };
  ...
  oauth.logout = function () {
    ...
    token('basic_auth_token', '');  // Basic Auth TODO
    return oauth;
  };
  ...
  oauth.fetch = function(resource, options) {
    ...
    function _doFetch() {
      ...
      options.headers.Authorization = 'Basic ' + token('basic_auth_token');// TODO
  ...
  oauth.xhr = function (options, callback) {
    ...
    function _doXHR() {
      ...
      return oauth.rawxhr(
        options.method,
        url,
        // `Bearer ${token('oauth2_access_token')}`,// TODO
        `Basic ${token('basic_auth_token')}`,// TODO
        options.content,
        options.headers,
        done
      );
